...
namespace PE
{
	...
	PE_FILE ParsePE(const char* PE)
	{
		...
		pefile.MS_DOS_STUB = shared_ptr<char>(new char[stub_size]);
		memcpy_s(pefile.MS_DOS_STUB.get(), stub_size, (PE + 0x3c + 0x4), stub_size);
		...
		pefile.ish = shared_ptr<IMAGE_SECTION_HEADER>{ new IMAGE_SECTION_HEADER[number_of_sections + 1]{} }; // Number of sections
		...
		{
			sections_size += pefile.ish.get()[i].SizeOfRawData;
		}
	...
	void Inject_into_Largest_Tail(char* pe_file, size_t size_of_pe, char xcode[], size_t size_of_xcode, const std::string& out_path)
	{
		...
		{
			if (Parsed_PE.ish.get()[i].SizeOfRawData > Parsed_PE.ish.get()[i].Misc.VirtualSize) // r_sz > v_sz
		...
		{
			if (Parsed_PE.ish.get()[i].SizeOfRawData - Parsed_PE.ish.get()[i].Misc.VirtualSize >= Parsed_PE.inh32.OptionalHeader.FileAlignment)
		...
		for (size_t i = 0; i < good_sections_r_sz.size(); ++i)
		{
			...
			{
				if (section[Parsed_PE.ish.get()[good_sections_r_sz[i]].Misc.VirtualSize + j] != 0x0)
				{
		...
		std::sort(good_sections_r_sz.begin(), good_sections_r_sz.end(), [&](int a, int b) {
			return Parsed_PE.ish.get()[a].SizeOfRawData - Parsed_PE.ish.get()[a].Misc.VirtualSize > Parsed_PE.ish.get()[b].SizeOfRawData - Parsed_PE.ish.get()[b].Misc.VirtualSize;
		});
		...
		if (good_sections_r_sz.empty() && !good_sections_v_sz.empty())
			...
			ready_section_index_size = tuple<WORD, size_t, size_t>{ good_sections_r_sz[0], Parsed_PE.ish.get()[good_sections_r_sz[0]].Misc.VirtualSize, Parsed_PE.ish.get()[good_sections_r_sz[0]].SizeOfRawData - Parsed_PE.ish.get()[good_sections_r_sz[0]].Misc.VirtualSize };
		else {
			if (Parsed_PE.ish.get()[good_sections_r_sz[0]].SizeOfRawData - Parsed_PE.ish.get()[good_sections_r_sz[0]].Misc.VirtualSize > get<2>(section_index_size_v_sz[0]))
				ready_section_index_size = tuple<WORD, size_t, size_t>{ good_sections_r_sz[0], Parsed_PE.ish.get()[good_sections_r_sz[0]].Misc.VirtualSize, Parsed_PE.ish.get()[good_sections_r_sz[0]].SizeOfRawData - Parsed_PE.ish.get()[good_sections_r_sz[0]].Misc.VirtualSize };
			else
				ready_section_index_size = tuple<WORD, size_t, size_t>{ get<0>(section_index_size_v_sz[0]), get<1>(section_index_size_v_sz[0]), get<2>(section_index_size_v_sz[0]) };
		}
		...
		DWORD characteristics = Parsed_PE.ish.get()[section].Characteristics;
		...
		Parsed_PE.ish.get()[section].Characteristics = characteristics;
		...
		auto AEP = index_ + Parsed_PE.ish.get()[section_].VirtualAddress;
		...
		if (Parsed_PE.ish.get()[section].SizeOfRawData > Parsed_PE.ish.get()[section].Misc.VirtualSize && Parsed_PE.ish.get()[section].Misc.VirtualSize != 0)
			Parsed_PE.ish.get()[section].Misc.VirtualSize += inj_size + 0x17;  // just enough
	...
	void Inject_into_code_tail(char* pe_file, size_t size_of_pe, char xcode[], size_t size_of_xcode, const std::string& out_path)
	{
		...
		for (WORD i = 0; i < Parsed_PE.inh32.FileHeader.NumberOfSections; ++i)
		{
			if (Parsed_PE.ish.get()[i].Characteristics & IMAGE_SCN_CNT_CODE)
			{
				code_section = { i };
				break;
			}
		...
		auto r_sz = Parsed_PE.ish.get()[code_section].SizeOfRawData > Parsed_PE.ish.get()[code_section].Misc.VirtualSize;
		if (r_sz) {
			...
			for (DWORD j = 0; j < size_zeros; ++j)
			{
				if (section[Parsed_PE.ish.get()[code_section].Misc.VirtualSize + j] != 0x0)
				{
			...
			ready_section_index_size = tuple<WORD, size_t, size_t>{ code_section, Parsed_PE.ish.get()[code_section].Misc.VirtualSize, Parsed_PE.ish.get()[code_section].SizeOfRawData - Parsed_PE.ish.get()[code_section].Misc.VirtualSize };
		...
		// If v_sz != 0 and v_sz < r_sz, increase v_sz by sizeof(xcode).
		if (Parsed_PE.ish.get()[code_section].SizeOfRawData > Parsed_PE.ish.get()[code_section].Misc.VirtualSize && Parsed_PE.ish.get()[code_section].Misc.VirtualSize != 0)
			Parsed_PE.ish.get()[code_section].Misc.VirtualSize += inj_size + 0x17;  // just enough
	...
	void Inject_Resize_Code(char* pe_file, size_t size_of_pe, char xcode[], size_t size_of_xcode, const std::string& out_path)
	{
		...
		auto Last_of_Original_Raw_Address = Parsed_PE.ish.get()[code_section].SizeOfRawData;
		auto aligned_size_of_xcode = align_up(inj_size, Parsed_PE.inh32.OptionalHeader.FileAlignment);
		Parsed_PE.ish.get()[code_section].SizeOfRawData += aligned_size_of_xcode; // resize size of code section


		for (size_t i = 0; i < Parsed_PE.inh32.FileHeader.NumberOfSections; ++i)
		{
			if (Parsed_PE.ish.get()[i].PointerToRawData > Parsed_PE.ish.get()[code_section].PointerToRawData)
			{
				Parsed_PE.ish.get()[i].PointerToRawData += aligned_size_of_xcode;
			}
		}
		...
		if (Parsed_PE.ish.get()[code_section].SizeOfRawData > Parsed_PE.ish.get()[code_section].Misc.VirtualSize && Parsed_PE.ish.get()[code_section].Misc.VirtualSize != 0) {
			...
			Parsed_PE.inh32.OptionalHeader.SizeOfImage = Parsed_PE.ish.get()[last_section].VirtualAddress + align_up((Parsed_PE.ish.get()[last_section].Misc.VirtualSize) ? Parsed_PE.ish.get()[last_section].Misc.VirtualSize : Parsed_PE.ish.get()[last_section].SizeOfRawData, Parsed_PE.inh32.OptionalHeader.SectionAlignment);
		}
